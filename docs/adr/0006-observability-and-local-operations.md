...
The default test suite remains external-service-free while metrics and logs are testable. The dashboard is reproducible configuration, not collected dashboard evidence. PostgreSQL/Redis/Redpanda connectivity, durable metrics/log shipping, and alert rules remain deferred in the default path. Bounded synthetic load and query evidence is opt-in and stored under `benchmark/results`; readiness currently reports simulated providers in standalone mode while Compose independently reports dependency container health.
