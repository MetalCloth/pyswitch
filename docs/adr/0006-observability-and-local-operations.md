...
The default test suite remains external-service-free while metrics and logs are testable. The dashboard is reproducible configuration, and `scripts/observability_evidence.py` provides one sanitized Compose verification of its provisioning and Prometheus datasource. PostgreSQL/Redis/Redpanda durability, metrics/log shipping, and alert rules remain deferred in the default path. Bounded synthetic load, query, consumer-offset, and observability evidence is opt-in and stored under `benchmark/results`; readiness reports the selected runtime dependencies in the explicit Compose profile.
