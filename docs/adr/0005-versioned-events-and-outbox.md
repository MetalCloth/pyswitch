...
`InMemoryBroker` is the default test double. `KafkaBroker` is an optional aiokafka adapter for a Redpanda/Kafka-compatible broker and is not wired into application startup. No external event effect is executed by the local consumer implementations.
