...
class Metrics:
    ...
    def __init__(self, registry: CollectorRegistry | None = None) -> None:
        ...
        self.provider_p95_latency_seconds = Gauge("pyswitch_provider_p95_latency_seconds", "Provider p95 latency from the bounded local window", ["provider"], registry=self.registry)
        self.provider_inflight = Gauge("pyswitch_provider_inflight", "Provider requests in flight", ["provider"], registry=self.registry)
        self.provider_timeouts_total = Counter("pyswitch_provider_timeouts_total", "Provider timeouts", ["provider"], registry=self.registry)
        self.provider_last_success_timestamp = Gauge("pyswitch_provider_last_success_timestamp", "Unix timestamp of the last provider success", ["provider"], registry=self.registry)
        self.provider_last_failure_timestamp = Gauge("pyswitch_provider_last_failure_timestamp", "Unix timestamp of the last provider failure", ["provider"], registry=self.registry)
